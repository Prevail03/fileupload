    ...
    if ($_FILES['file']['error'] === UPLOAD_ERR_OK) {
        ...
        try {
            ...
            foreach ($worksheet->getRowIterator(2) as $row) {
                ...
                // Perform data validation for specific columns
                $accountNumber = $rowData[0];
                if (!is_string($accountNumber)) {
                    $errors[] = "Invalid account number at row {$row->getRowIndex()}";
                }

                $accountName = $rowData[1];
                if (!is_string($accountName)) {
                    $errors[] = "Invalid account name at row {$row->getRowIndex()}";
                }

                $address = $rowData[2];
                if (!is_string($address)) {
                    $errors[] = "Invalid address at row {$row->getRowIndex()}";
                }

                $currency = $rowData[3];
                if (!is_string($currency)) {
                    $errors[] = "Invalid currency at row {$row->getRowIndex()}";
                }

                $date = $rowData[4];
                if (!is_string($date)) {
                    $errors[] = "Invalid date at row {$row->getRowIndex()}";
                }

                $description = $rowData[5];
                if (!is_string($description)) {
                    $errors[] = "Invalid description at row {$row->getRowIndex()}";
                }

                $withdrawal = $rowData[6];
                if (!is_numeric($withdrawal)) {
                    $errors[] = "Invalid withdrawal amount at row {$row->getRowIndex()}";
                }

                $deposit = $rowData[7];
                if (!is_numeric($deposit)) {
                    $errors[] = "Invalid deposit amount at row {$row->getRowIndex()}";
                }

                $balance = $rowData[8];
                if (!is_numeric($balance)) {
                    $errors[] = "Invalid balance amount at row {$row->getRowIndex()}";
                }
            ...
            if (!empty($errors)) {
                // Handle the errors, for example, by displaying them to the user
                foreach ($errors as $error) {
                    echo $error.'<br>';
                }
                header('location: importBankDetails.php?validationfailure');
            } else {
                // Data is valid, proceed with inserting into the database
                // ...
