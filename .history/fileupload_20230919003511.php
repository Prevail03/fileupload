...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if ($_FILES['file']['error'] === UPLOAD_ERR_OK) {
        ...
        $sql = 'INSERT INTO assetInventory (laptopID, machineID , name, laptopModel, laptopSN, yearofpurchase, valueOfPurchase, usageStatus, whereItIs, insuranceStatus , currentOwner, statusOfMachine, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
        ...
        foreach ($worksheet->getRowIterator(2) as $row) {
            ...
            $laptopID = bin2hex(random_bytes(6));
            $machineID = $rowData[0];
            $name = $rowData[1];
            $laptopMode = $rowData[2];
            $laptopSN = $rowData[3];
            $yearofpurchase = $rowData[4];
            $valueOfPurchase = $rowData[5];
            $usageStatus = $rowData[6];
            $whereItIs = $rowData[7];
            $insuranceStatus = $rowData[8];
            $currentOwner = $rowData[9];
            $statusOfMachine = $rowData[10];

            $params = [$laptopID, $machineID, $name, $laptopModel, $laptopSN, $yearofpurchase, $valueOfPurchase, $usageStatus, $whereItIs, $insuranceStatus, $currentOwner, $statusOfMachine, $createdAt];
        ...
        <div class="row">
          ...
          <div class="col-md-6">
            ...
            <div class="card card-primary" style=" width: 1000px">
              ...
              <!-- form start -->
              <h1>File Upload</h1>
              <form action="uploadinventory" method="post" enctype="multipart/form-data">
